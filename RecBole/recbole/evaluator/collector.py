...
class DataStruct(object):
    ...
    def update_array(self, name: str, value):
        if name not in self._data_dict:
            self._data_dict[name] = value
        else:
            if not isinstance(self._data_dict[name], np.ndarray):
                raise ValueError("{} is not a tensor.".format(name))
            self._data_dict[name] = np.concatenate(
                (self._data_dict[name], value), axis=0
            )
...
class Collector(object):
    ...
    def eval_batch_collect(
        ...
        if self.register.need("data.pos_items"):
            res = np.empty([scores_tensor.shape[0]], dtype=object)
            for u, i in zip(positive_u, positive_i):
                if res[u] is None:
                    res[u] = np.array([i])
                else:
                    res[u] = np.append(res[u],np.array([i]))
            self.data_struct.update_array("data.pos_items", res)
    ...
    def get_data_struct(self):
        ...
        for key in ["rec.topk", "rec.meanrank", "rec.score", "rec.items", "data.label", "data.pos_items"]:
